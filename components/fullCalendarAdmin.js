      ...
      for (var i = 0; i < events.length; i++) {
        if (events[i].start === arg.dateStr) {
          ...
          if (events[i].eventType === "Quiz") {
            if (events[i].classroom === true) {
              if (events[i].status == "Active") {
                ...
                // 
                display.push(
                  `<h2>${events[i].title}</h2>  <b>Start Time</b> :- ${events[i].start_time}    <b>End Time</b> :- ${events[i].end_time} <br> <b>Status</b> :-  ${events[i].blocked ? "Blocked" : events[i].status} <br/> <a href="/workshop/WorkshopQuizDetails?q_id=${events[i].token}"><button style="background-color: #4CAF50; padding:10px;border:none; color: white;">Quiz Details</button></a> 
                <hr/>
          ...
          if (events[i].classroom === true) {
            ...
            display.push(`<h2>${events[i].title}</h2>  <b>Due Date</b> :- ${events[i].start}  <a href="/workshop/WorkshopTaskDetails?id=${events[i].token}"><button>Task Details</button></a>               <hr/>
`)
          }
          else {
            display.push(`<h2>${events[i].title}</h2>  <b>Due Date</b> :- ${events[i].start}  <a href="/workshop/WorkshopTaskDetails?id=${events[i].w_id}&t_id=${events[i].token}"><button>Task Details</button></a>                <hr/>
`)
          }
        }
        else {
          ...
          if (enable === 0) {
            ...
            display.push(`<h2>${events[i].title}</h2>
                              <a><button disabled style="background-color: grey;  color: white">Meet Expired</button></a> Expired               <hr/>
            ...
            display.push(`<h2>${events[i].title}</h2> 
               <button 
               style="background-color: #4CAF50; padding:15px;border:none; color: white;">Class Started</button>          
     <hr/>`
...
const startMeet = (id) => {
 ...
 const enableMeet = (item) => {
   ...
   var meetstartMinutes = parseInt(item.time.slice(2, 5));
   var meetEndMinutes = parseInt(item.end_time.slice(2, 5));

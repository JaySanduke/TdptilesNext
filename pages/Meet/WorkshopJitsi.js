...
class App extends Component {
 ...
 {this.state.work_id && 
                     ...
                     firebase.database().ref("Workshops/" + this.state.work_id + "/data/").once("value").then((snapshot)=>{
                        var workData = snapshot.val();
                        this.setState({classRating : workData.rating ? workData.rating.classRating : 0});
                        this.setState({users : workData.rating ? workData.rating.users : 0});
        ...
        firebase.auth().onAuthStateChanged((user) => {
            firebase.database().ref(`users/${user.uid}`).once("value")
                .then((snap) => {
                    ...
                    firebase.database().ref("Workshops/" + this.state.class_id + "/data/meet/" + this.state.meet_id + "/data/").once('value').then((parentsnapshot) => {
    ...
    onStarClick(nextValue, prevValue, name) {
        ...
        firebase.database().ref("Workshops/" + this.state.work_id + "/data/rating").set({

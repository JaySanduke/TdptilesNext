...
class AdminCalendar extends React.Component {
    ...
    deleteMeet = (id) => {
        firebase.database().ref(`Workshops/${this.state.c_id}/data/meet`).child(id).remove();
    ...
    enableMeet = (id) => {
        firebase.database().ref(`Workshops/${this.state.c_id}/data/meet/${id}/data`).update({
            meet_started: 1,
        })
    }
    ...
    handleSubmit = (e) => {
        ...
        if (this.state.title === "") alert("Enter Title");
            ...
            firebase
                .database()
                .ref(`Workshops/${this.state.c_id}/data/meet/${meet_id}`)
                .set({
                    data: x,
                }).then(() => {
    ...
    var todayDay = parseInt(today.getDate());
        ...
        const body = this.state.meet_data.map((item) => {
            ...
            return {
                ...
                joinBtn: ( enableJoin ? 
                    <a href={"/Meet/jitsi?id=" + item.data.id}><Button onClick={() => this.enableMeet(item.data.id)} color="success">Join Meet</Button></a> : <Button>{msg}</Button>
                )
            }
        })
...
AdminCalendar.layout = MentorWorkshop;

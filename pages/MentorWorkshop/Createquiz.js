  ...
  handleSubmit = (e) => {
    ...
    if (!this.isSubmitAllowed()) {
      ...
      if (this.state.quiz_data[0]) {
        ...
        firebase
          .database()
          .ref("Quiz/workshopQ/" + cQid + "/data/").update({
        ...
        firebase
          .database()
          .ref("Quiz/workshopQ/" + cQid + "/data/")
          .update({
            quiz_id: cQid,
          })
          .then(() => {
            alert("Quiz Created Sucessfully!");
            window.location.href = `/MentorWorkshop/Workshopquiz?id=${this.state.w_id}`;
          });
      } else {

...
class WorkshopEditQuiz extends React.Component {
  constructor(props) {
    super(props);
     var today = new Date(),
      todayDate =
      ...
      firebase.database().ref(`mentors/${user.uid}`).once("value")
        .then((snap) => {
          ...
          if (d && d.type === 2) {
            ...
            this.setState({ quiz_id: id, w_id: w_id });
  ...
  handleSubmit = (e) => {
    ...
    if (value !== null &&
      ...
      if (this.state.quiz_data[0]) {
        ...
        db.ref("Quiz/workshopQ/" + this.state.quiz_id + "/data/")
          .update({
          ...
          .then(() => {
            ...
            window.location.href = `/MentorWorkshop/WorkshopQuizDetails?id=${this.state.w_id}&q_id=${this.state.quiz_id}`;
          });
      } else {
...
WorkshopEditQuiz.layout = Workshop;
export default WorkshopEditQuiz;

      ...
      firebase.database().ref(`mentors/${user.uid}`).once("value")
        .then((snap) => {
          ...
          if (d && d.type === 2) {
            ...
            var ref = firebase.database().ref("Classroom/" + id + "/data/");
            var x = [];
            ref.once("value").then((snapshot) => {
              ...
              if ((dat === null) ? dat : []) {
                if (dat.quiz) {
                  ...
                  // console.log("123"+this.state.quizes)
                }
              }
            });
            const db = firebase.database();
            const refz = db.ref("Classroom/" + this.state.c_id + "/data/");
            refz.once("value").then((snapshot) => {

              var dataz = snapshot.val();
              // console.log(dataz)
              this.setState({ cname: dataz.classroomName });

              // z.push({c_id:this.state.c_id,classroomName: this.state.cname})
            });
  ...
  handleSubmit = (e) => {
    ...
    if (!this.isSubmitAllowed()) {
      ...
      if (this.state.quiz_data[0]) {
        ...
        var cQid = firebase.database().ref("blogs").push().key;


        firebase
          .database()
          .ref("Quiz/classQ/" + cQid)
          .set({
            data: {
              ...
              quiz_desc: value,
              classrooms: [{ c_id: this.state.c_id, classroomName: this.state.cname }],
              duration: this.state.duration,
              updatedon:"Not Yet Updated",
              mentor:this.state.mentorId,
            },
          });



        var ref = firebase.database().ref("Quiz/classQ" + cQid + "/data/");
        var y = [];
        ref.once("value").then((snapshot) => {
          var daty = snapshot.val();
          if ((daty === null) ? daty : []) {
            if (daty.classrooms) {
              y = daty.classrooms;
              this.setState({ classes: y });
              // console.log("123"+this.state.quizes)
            }
          }
        });
        ...
        firebase
          .database()
          .ref("Classroom/" + this.state.c_id + "/data/")
          .update({
            quiz: x,
          });
        firebase
          .database()
          .ref("Quiz/classQ/" + cQid + "/data/")
          .update({
            quiz_id: cQid,
          })
          .then(() => {
            alert("Quiz Created Sucessfully!");
             window.location.href = `/MentorClass/dashboard`;
          });}
      } else {
...
CreateQuiz.layout = MentorClassroom;

...
class WorkshopCreateTask extends React.Component {
      ...
      firebase.database().ref(`mentors/${user.uid}`).once("value")
        .then((snap) => {
         ...
         let id = localStorage.getItem("id");
            ...
            const refz = db.ref("Classroom/" + id + "/data/");
  ...
  handleSubmit = (e) => {
    ...
    if (!this.isSubmitAllowed()) {
      ...
      firebase.database().ref("Classroom/" + this.state.c_id + "/data/").once("value").then((s) => {
        var data = s.val();
        var x = data.task;
        if (x === undefined) {
          x = [];
        ...
        .then(() => {
          ...
          window.location.href = '/MentorClass/dashboard?id=' + this.state.c_id;
        });
    }
  };
          ...
          <>
            ...
            <Container className="mt--7" fluid>
              ...
              width={200}></Lottie> : <Input
                  ...
                  {/* <Button className="d-none" id="routeBtn"><Link to={{ pathname: "/workshop/workshopdetails", state: { id: this.state.c_id } }}>
              View
            </Link></Button> */}
...
WorkshopCreateTask.layout = MentorClassroom;

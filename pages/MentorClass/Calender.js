...
// DemoApp.layout = MentorClassroom;
...
import FullCalendar from "../../components/fullCalendarmentor";
...
import MentorClassroom from "layouts/MentorClassroom.js";
import { useState } from "react";
import Header from "components/Headers/Calendar.js";
...
class DemoApp extends React.Component {
                  ...
                  .then((snapshot) => {
                    ...
                    if (childData) {
                      var x = {
                        ...
                        classroom: true,
                        status: active ? "Active" : "Inactive",
                        blocked:childData.block,
                      };
                      events.push(x);

                      index++;
                      if(active){
                      tts.setState({quizes:tts.state.quizes+1});}
                      //console.log(tts.state.quizes)
                    }
                    tts.setState({
                      arr: [...tts.state.arr, ...events],
                    });
              ...
              .then((snapshot) => {
                ...
                snapshot.forEach(function (childSnapshot) {
                  ...
                  if (isTask) {
                    ...
                    events.push(x);
                    index++;
                   if(tts.state.todayDate < childData.data.due_date){
                     tts.setState({tasks:tts.state.tasks+1});}
                  }
                });
                this.setState({
                  arr: [...this.state.arr, ...events],
              ...
              .then((snapshot) => {
                ...
                snapshot.forEach(function (childSnapshot) {
                  ...
                  if (childData) {
                    ...
                    if(tts.state.todayDate< childData.data.date){
                     tts.setState({classes:tts.state.classes+1});}
                  }
                });
                // console.log(events);
                this.setState({
  ...
  render() {
    return (
      <div className="MainDiv">
        <Header quiz={this.state.quizes} class={this.state.classes} task={this.state.tasks} />
        <Container>
          <Card className="mt--7 p-4">
            <FullCalendar
              // plugins={[dayGridPlugin, interactionPlugin]}
